...
namespace TestLab10
{
    ...
    public class ProductionTests
    {
        ...
        [TestMethod]
        public void Production_Employees_ShouldNotBeNegative()
        {
            // Arrange
            Production production = new Production();
            ...
            // Assert
            Assert.AreEqual(0, production.Employees, "Employees не должен быть отрицательным.");
            StringAssert.Contains(input.ToString(), "Ошибка: Количество работников не может быть отрицательным", "Сообщение об ошибке должно быть выведено.");
        }
    }
}
